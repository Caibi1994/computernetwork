...
class Wechat {
    ...
    public function response($content, $type = self::MSG_TYPE_TEXT){
        ...
        $userInfo = M('student_info')->where(array('openId' => $data['ToUserName']))->find();
        
        $messageContent = '';
        if(array_key_exists('Content',$data))
            $messageContent = $data['Content'];
        else if(array_key_exists('ArticleCount', $data)){
            foreach ($data['Articles'] as $key => $value) {
                $messageContent .= implode(',', $data['Articles'][$key]);
                $messageContent .= '###' ;  //分隔符
            }    
        } 
        else 
            $messageContent = '该信息不是图文消息和文本消息'.json_encode($data);
